		...
		const chart = new Chart(itemsDonutCanvas, {
			...
			options: {
				...
				plugins: {
					...
					tooltip: {
						callbacks: {
							label: (ctx) => ` ${ctx.label}: ${ctx.raw} (${(((ctx.raw as number) / totalItems) * 100).toFixed(0)}%)`
						}
					}
				}
			},
		...
		const chart = new Chart(spendingDonutCanvas, {
			...
			options: {
				...
				plugins: {
					...
					tooltip: {
						callbacks: {
							label: (ctx) => ` ${ctx.label}: €${(ctx.raw as number).toFixed(2)} (${(((ctx.raw as number) / totalSpent) * 100).toFixed(0)}%)`
						}
					}
				}
			},
			plugins: [makeCenterTextPlugin(`€${totalSpent.toFixed(0)}`, get(_)('analytics.totalSpent'))]
		});
		return () => chart.destroy();
	});
